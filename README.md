...
Status: **0.15.0.** Consistent under HermiT, structurally validated, unit-checked against QUDT.
...
each check with something that empties what it traverses — the schema with no example
data, an empty graph and no example files where the population is the schema itself, or
an example file holding nothing where the check re-reads `examples/` — and requires every
...
BASELINE.json` classifies a change against a consumer's pinned baseline.

A consumer's pin still belongs to the consumer, and ThermalEdge holds its own.
FMO holds one too — `shapes/thermaledge-export.pin.json`, generated by
`--update` and audited by `--audit` — because FMO is the party that edits the
shapes file, and publishing a classifier nobody points at the file leaves the
weakening to be found downstream, a nightly run later. Widening
`fm:probabilityValue` from `0..1` to `-1..1.5`, the range guarding the 7.41 bug
below, passed all eleven of this repo's non-Java targets. `make shape-signatures`
checks the signatures reproduce, runs the drift mutants, then audits the
signatures against the pin; `make shape-signatures-update` re-pins an intended
change as a reviewed diff. See `docs/adr/0002-pin-the-export-contract.md`.
...
and that is the policy `--audit` applies to FMO's own pin, since the range
widening above lands in that second bucket. `--compare` itself exits 0 whenever
it produced a report, because what a verdict is worth is the caller's decision,
and only the audited pin is FMO's to decide. A SHACL construct the signer does not model fails the signer,
