...
weakening to be found downstream, a nightly run later. It was found that way:
widening `fm:probabilityValue` from `0..1` to `-99..99`, the range guarding the
7.41 bug below, passed every check this repo had. `make shape-signatures` checks
the signatures reproduce, audits them against the pin, and runs the drift
mutants; `make shape-signatures-update` re-pins an intended change as a reviewed
diff. See `docs/adr/0002-pin-the-export-contract.md`.
